...
class DomainTests(common.IdentityTests):
    def test_domain_create(self):
        ...
        raw_output = self.fibostack('domain create %s' % domain_name)
    ...
    def test_domain_list(self):
        ...
        raw_output = self.fibostack('domain list')
    ...
    def test_domain_delete(self):
        ...
        raw_output = self.fibostack('domain set --disable %s' % domain_name)
        self.assertEqual(0, len(raw_output))
        raw_output = self.fibostack('domain delete %s' % domain_name)
        self.assertEqual(0, len(raw_output))
    ...
    def test_domain_multi_delete(self):
        ...
        # cannot delete enabled domain, disable it first
        raw_output = self.fibostack('domain set --disable %s' % domain_1)
        self.assertEqual(0, len(raw_output))
        raw_output = self.fibostack('domain set --disable %s' % domain_2)
        self.assertEqual(0, len(raw_output))
    ...
    def test_domain_show(self):
        domain_name = self._create_dummy_domain()
        raw_output = self.fibostack('domain show %s' % domain_name)
        items = self.parse_show(raw_output)
        self.assert_show_fields(items, self.DOMAIN_FIELDS)
